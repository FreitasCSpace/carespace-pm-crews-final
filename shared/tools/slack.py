...
def post_standup(done: str, in_progress: str, blocked: str, attention: str) -> str:
    """
    Posts the structured daily standup to #standup.
    ...
    r = _api("#standup", f"Standup {today}", [
        _hdr(f"Daily Standup -- {today}"),
        _sec(body),
        _ctx("_Posted by CareSpace Scrum AI_"),
    ])
...
def post_sprint_plan(sprint_name: str, tasks_json: str, total_sp: int) -> str:
    """
    Posts the sprint planning results to #sprint-board.
    ...
    r = _api("#sprint-board", f"Sprint plan: {sprint_name}", [
        _hdr(sprint_name),
        _sec(f"*{len(tasks)} tasks committed - {total_sp} story points*"),
        _div(),
        _sec("\n".join(lines) or "_No tasks selected_"),
        _ctx("_Sprint plan generated by CareSpace Scrum AI_"),
    ])
    return json.dumps({"ok": r.get("ok")})
...
def post_sla_breach(bug_name: str, task_url: str, priority: str, hours_open: int) -> str:
    """
    Posts an SLA breach alert to #alerts for bugs that exceeded their resolution window.
    """
    sla = {"urgent": 4, "high": 24, "normal": 72, "low": 168}.get(priority, 24)
    r = _api("#alerts", f"SLA BREACH: {bug_name}", [
        _sec(
...
def post_retro(sprint_name: str, completion_pct: float,
    ...
    """Posts end-of-sprint retrospective summary to #sprint-board."""
    r = _api("#sprint-board", f"Retro: {sprint_name}", [
        ...
        _ctx("_Retrospective by CareSpace Scrum AI_"),
    ])
    return json.dumps({"ok": r.get("ok")})
...
def post_gtm(headline: str, deals_at_risk: int, pipeline_value: str,
    ...
    """Posts the weekly GTM pipeline intelligence report to #gtm."""
    r = _api("#gtm", "GTM Pipeline Update", [
...
def post_exec(report_summary: str) -> str:
    """Posts the weekly cross-workspace executive summary to #exec-updates."""
    r = _api("#exec-updates", "Weekly Status Report", [
        _hdr(f"CareSpace Weekly Status -- {date.today().strftime('%B %d, %Y')}"),
        _sec(report_summary),
        _ctx("_Generated by CareSpace Scrum AI_"),
    ])
    return json.dumps({"ok": r.get("ok")})
...
def post_compliance(control_name: str, severity: str, finding: str) -> str:
    """Posts a compliance control gap or failure alert to #compliance."""
    r = _api("#compliance", f"Compliance: {control_name}", [
        _sec(
...
def post_cs_alert(account_name: str, risk_type: str, detail: str) -> str:
    """Posts a customer success risk alert to #customer-success."""
    r = _api("#customer-success", f"CS Alert: {account_name}", [
